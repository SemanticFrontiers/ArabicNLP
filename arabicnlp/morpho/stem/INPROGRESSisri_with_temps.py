...
class ISRIStemmer(StemmerI):
    ...
    def pprint(self, token):
        print u'%s: %s على الوزن %s' % (token, self.stem(token), self.pattern(token))

    def _find_missing(self, letter, possibilities):
        return possibilities[possibilities.index(letter)]
    ...
    def pro_w4(self):
        ...
        if self.stm[0] in self.pr4[0]:      #  مفعل
            ...
            self.pttrn = u'فع%sل' % self._find_missing(self.stm[2], self.pr4[2])
    ...
    def pro_w54(self):
        ...
        if (self.stm[0] in self.pr53[2]):       #تفعلل - افعلل - مفعلل
            self.pttrn = u'%s\u0641\u0639\u0644\u0644' % self._find_missing(self.stm[0], self.pr53[2])
            self.stm = self.stm[1:]
            return self.stm
...
if __name__ == "__main__":
    ...
    if errors:
        ...
        for t in errors:
            e.pprint(t)
        raise ValueError("Almost there! [%d/%d]" % (len(toks) - len(errors), len(toks)))
    import doctest
